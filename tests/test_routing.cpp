...
void test_vehicle_plan_route() {
    ...
    Graph graph;
    for (int i = 1; i <= 6; ++i) graph.add_node(i);
    graph.add_edge(1, 1, 2, 7.0);
    graph.add_edge(2, 1, 3, 9.0);
    graph.add_edge(7, 3, 6, 2.0);
    graph.add_edge(9, 6, 5, 9.0);
    // Path 1 -> 3 -> 6 -> 5 has weight 9+2+9 = 20
    // Direct edge 1 -> 5 (if added) would need to be > 20 for 1-3-6-5 to be shortest.
    // For this test, we rely on the graph setup providing a known shortest path.

    // Test 1: Vehicle plans a valid route
    Vehicle vehicle1(1, 1, 5); // Wants to go from node 1 to node 5
    vehicle1.plan_route(graph);
    std::vector<int> expected_path1 = {1, 3, 6, 5}; // Based on find_shortest_path logic
    ...
    graph.add_node(10); // Add a disconnected node
    ...
    // find_shortest_path handles this by returning empty.
    Vehicle vehicle4(4, 99, 1); // Node 99 does not exist in graph
    ...
    Vehicle vehicle5(5, 1, 88); // Node 88 does not exist in graph
